...
class AutoReturnApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.gmail_service = GmailIntegrationService(data_dir=self._get_gmail_data_dir())
        
        # Initialize AI service with cloud model for better performance
        ...
        self.gmail_refresh_timer.start(60000)
    ...
    def sync_all_messages(self):
        """Synchronize all messages from connected services."""
        any_source = False

        if self.slack_service.is_connected:
            any_source = True
            print("Syncing Slack messages...")
            new_messages = self.slack_service.fetch_all_messages(limit=100)
            if new_messages:
                self.on_slack_new_messages(new_messages)
                print(f"Synced {len(new_messages)} new Slack messages")
            else:
                print("Slack sync complete - No new messages")

        if self.gmail_service.is_connected:
            any_source = True
            print("Syncing Gmail messages...")
            self.handle_gmail_sync(quiet=True)
        ...
        if not any_source:
            QMessageBox.warning(
                self,
                "Not Connected",
                "Please connect to Slack or Gmail first.\n\nGo to Settings → Integrations."
            )
    ...
    def on_gmail_new_messages(self, messages: list):
        ...
        Args:
            messages (list): List of new message dictionaries
        """
        if not messages:
            return
        print(f"Processing {len(messages)} Gmail messages")
    ...
    def handle_gmail_sync(self, quiet: bool = False):
        """Synchronize messages from Gmail.
        
        Args:
            quiet (bool): If True, suppress success notifications
        """
        try:
            emails = self.gmail_service.sync_messages(max_results=10)
            message = f"Fetched {len(emails)} Gmail message(s)."
            if not quiet:
                QMessageBox.information(self, "Gmail Sync", message)
            return True, message
        except Exception as exc:
            if not quiet:
                QMessageBox.warning(self, "Gmail Sync", str(exc))
    ...
    def create_main_content(self):
        ...
        filter_buttons = [
            ("All", "all", None),
            ("Gmail", "gmail", os.path.join(os.path.dirname(__file__), "assets", "Gmail_Logo_32px.png")),
            ("Slack", "slack", os.path.join(os.path.dirname(__file__), "assets", "icons8-slack-new-48.png")),
            ("Urgent", "urgent", os.path.join(os.path.dirname(__file__), "assets", "notification-bell-red.png"))
        ]
        ...
        try:
            ...
            for row, msg in enumerate(filtered):
                ...
                if msg.get('source') == 'gmail':
                    pixmap = QPixmap(os.path.join(os.path.dirname(__file__), "assets", "Gmail_Logo_32px.png"))
                else:
                    pixmap = QPixmap(os.path.join(os.path.dirname(__file__), "assets", "icons8-slack-new-48.png"))
